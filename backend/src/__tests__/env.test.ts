...
const SERVICES: ServiceConfig[] = [
  { name: "BE", portEnv: "DASHBOARD_PORT", portStart: 5111, portStep: 10 },
...
describe("allocatePorts", () => {
  it("returns index-1 ports when no existing worktrees", () => {
    ...
    expect(result).toEqual({ DASHBOARD_PORT: "5121", FRONTEND_PORT: "5122" });
  });

  it("skips occupied indices", () => {
    const existing = [
      { DASHBOARD_PORT: "5121" }, // slot 1
      { DASHBOARD_PORT: "5131" }, // slot 2
    ];
    const result = allocatePorts(existing, SERVICES);
    expect(result).toEqual({ DASHBOARD_PORT: "5141", FRONTEND_PORT: "5142" });
  });

  it("fills gaps in occupied indices", () => {
    const existing = [
      { DASHBOARD_PORT: "5131" }, // slot 2 (gap at slot 1)
    ];
    const result = allocatePorts(existing, SERVICES);
    expect(result).toEqual({ DASHBOARD_PORT: "5121", FRONTEND_PORT: "5122" });
  });
  ...
  it("returns empty when no services have portStart", () => {
    const services: ServiceConfig[] = [
      { name: "BE", portEnv: "DASHBOARD_PORT" },
    ];
    const result = allocatePorts([], services);
  ...
  it("ignores port values not aligned to portStep", () => {
    const existing = [
      { DASHBOARD_PORT: "5115" }, // not aligned to step 10 from 5111
    ];
    const result = allocatePorts(existing, SERVICES);
    expect(result).toEqual({ DASHBOARD_PORT: "5121", FRONTEND_PORT: "5122" });
  });
  ...
  it("does not assign slot 0 (reserved for main)", () => {
    ...
    const result = allocatePorts([], SERVICES);
    expect(Number(result.DASHBOARD_PORT)).toBeGreaterThan(5111);
