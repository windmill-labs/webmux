...
<dialog bind:this={dialogEl} onclose={onclose} class="bg-sidebar text-primary border border-edge rounded-xl p-6 max-w-[560px] w-[90%]">
    ...
    {#each pr.ciChecks as check (check.name + check.runId)}
      <li class="rounded-md border border-edge bg-surface p-3">
        <div class="flex items-center gap-2">
          <span class="text-sm font-bold {statusColor(check.status)}">{statusIcon(check.status)}</span>
          <span class="text-[13px] font-medium flex-1 truncate">{check.name}</span>
          <span class="text-[11px] {statusColor(check.status)}">{check.status}</span>
        </div>
        <div class="flex items-center gap-2 mt-1.5">
          {#if check.status === "failed" && check.runId}
            <button
              type="button"
              class="text-[11px] text-accent cursor-pointer bg-transparent border-none p-0 hover:underline"
              onclick={() => handleViewLogs(check.runId)}
            >{expandedRunId === check.runId ? "Hide logs" : "View logs"}</button>
          {/if}
          {#if check.url}
            <a
              href={check.url}
        ...
        {#if expandedRunId === check.runId}
          <div class="mt-2">
            ...
            {:else if logs}
              <pre class="bg-surface border border-edge rounded-md p-3 text-[11px] font-mono overflow-x-auto max-h-[300px] overflow-y-auto whitespace-pre-wrap m-0">{logs}</pre>
